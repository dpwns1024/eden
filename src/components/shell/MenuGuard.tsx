 ...
 * - 비로그인 방문자: URL, 게시판 권한 상관없이 1번 비밀번호 입력창 강제 노출
 */
...
function GuardInner({ children }: { children: React.ReactNode }) {
  ...
  // 1. 세션 스토리지 통과 내역 확인 (브라우저 추적 방지 기능 에러 방어)
  useEffect(() => {
    try {
  ...
  // 💡 [해결 1] 관리자 및 로그인 사용자는 홈 비번 무조건 통과 후 개별 메뉴 권한 체크
  if (user || isAdmin) {
  ...
  // 💡 [해결 2 & 3] 비로그인 방문자 대상 홈 비번 검사 (게시판 '전체공개(all)' 여부와 상관없이 무조건 1순위 적용)
  ...
  // 비로그인 방문자이고 홈 비번이 걸려있는데 세션 통과(isPassed)를 안 했다면?
  // 게시판이 'all'로 설정되어 있어도 무조건 여기서 막고 모달 노출!
  ...
  // 3. 비로그인이지만 홈 비번을 이미 입력해 통과(isPassed=true)했거나 비번이 설정 안 된 경우에만 개별 메뉴 권한 검사
  const vis = hrefAccess(menuSet, path);
  const ok = vis === 'all' || (vis === 'member' && !!user) || (vis === 'admin' && isAdmin);
  if (ok) return <>{children}</>;
