...
export default function BoardDetailPage() {
  ...
  // ★ 레이아웃 변형 없이 동적으로 파일 업로드/정화 클릭 이벤트 바인딩
  useEffect(() => {
    if (!html) return;

    const handleClick = (e: MouseEvent) => {
      const target = e.target as HTMLElement;
      const box = target.closest('#stegano-box');
      if (box) {
        const fileInput = document.getElementById('exif-input-v5') as HTMLInputElement | null;
    ...
    const handleChange = (e: Event) => {
      ...
      if (target && target.id === 'exif-input-v5') {
        const files = target.files;
        const downloadArea = document.getElementById('download-area-v5');
        if (!downloadArea || !files || files.length === 0) return;

        downloadArea.innerHTML = '<p style="color:#673AB7; font-weight:bold; margin-top:10px;">✨ 픽셀 정화 완료! 아래 버튼을 눌러 다운로드하세요.</p>';
